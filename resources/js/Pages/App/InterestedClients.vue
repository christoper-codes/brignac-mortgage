...
<template>
    ...
    <AppLayout>
        ...
        <div class="p-4 lg:p-10 lg:mx-10 bg-white shadow-lg text-center mt-7 rounded-md">
            <h2 class="font-bold text-xl lg:text-4xl text-gray-500"><span class="italic text-green-500">Our</span> interested clients</h2>
            <div class="mt-5 border-l-8 border-l-green-500 bg-green-100 p-7 text-sm lg:text-base">
                <span class="font-bold">Hello administration 👋.</span> These are the details of the queries that our potential clients interested in a program ask us.
            </div>
        </div>
        ...
        <div class="my-10 p-4 lg:p-10">
            <v-data-table color="black" :items="items" :headers="headers" :header-props="headerProps">
                <template v-slot:item.Customer="{ item }">
                    <span class="py-1 px-4 rounded-full" :class="item.Customer === 'No' ? '!text-green-600 bg-green-100' : '!text-blue-600 bg-blue-100'">
                        {{ item.Customer }}
                    </span>
                </template>
                <template v-slot:item.Files="{ item }">
                    ...
                    <v-list v-else>
                        <span class="text-red-500">files not included</span>
                    </v-list>
                </template>
            </v-data-table>
        </div>
