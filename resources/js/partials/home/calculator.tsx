...
const RATE_SOURCES = [
    ...
    {
        label: 'www.bankrate.com',
        href: 'https://www.bankrate.com/mortgages/mortgage-rates/',
    },
    {
        label: 'money.usnews.com',
        href: 'https://money.usnews.com/loans/rates/mortgages/mortgage-rates',
    },
];
...
export function Calculator() {
    ...
    const yearly = useMemo(
        () => (result ? toYearlySchedule(result.schedule) : []),
        [result],
    );
    ...
    const totals = useMemo(() => {
        ...
        return {
            ...
            payoffLabel: payoff.toLocaleDateString('en-US', {
                month: 'short',
                year: 'numeric',
            }),
            downPayment: Math.max(
                0,
                result.params.property - result.params.loan,
            ),
            ltv:
                result.params.property > 0
                    ? (result.params.loan / result.params.property) * 100
                    : 0,
        };
    }, [result]);
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                    {[
                                        ...
                                        {
                                            label: 'Loan-to-Value',
                                            value: `${totals.ltv.toFixed(1)}%`,
                                        },
                                        {
                                            label: 'Rate / Term',
                                            value: `${result.params.rate.toFixed(2)}% · ${result.params.years} yrs`,
                                        },
                                    ].map((card) => (
                                        <div
                                            key={card.label}
