...
function PrequalCard() {
    return (
            <div className="relative flex min-h-[381px] flex-col overflow-hidden rounded-[15px] md:h-full md:min-h-0 md:w-[52.2%] md:shrink-0">
                ...
                <div className="relative flex h-full flex-col p-6 sm:p-10 md:p-[50px]">
                    ...
                    <motion.div
                        ...
                        className="mt-8 w-full max-w-[328px] self-center md:mt-[69px]"
                    >
        ...
        <div className="relative flex min-h-[450px] flex-col overflow-hidden rounded-[20px] ring-1 ring-white/10 md:h-full md:min-h-0">
            ...
            <div className="relative flex h-full flex-col p-7 sm:p-10 md:p-[45px]">
                <motion.div
                    ...
                    className="pointer-events-none absolute top-[63px] left-[-18px] z-0 hidden w-[242px] items-center gap-2.5 rounded-[11px] bg-white/60 p-2.5 backdrop-blur-sm md:top-[172px] md:left-[18px] md:flex"
                >
                ...
                <motion.div
                    ...
                    className="pointer-events-none absolute top-[204px] right-[-6px] z-0 hidden w-[242px] items-center gap-2.5 rounded-[11px] bg-white/60 p-2.5 backdrop-blur-sm md:top-[194px] md:right-[20px] md:flex"
                >
                    <span className="grid h-[38px] w-[38px] shrink-0 place-items-center rounded-full bg-white text-primary shadow-[0_1px_5px_0_rgba(90,91,107,0.09),inset_2px_7px_2px_0_rgba(255,255,255,0.4)]">
                ...
                <div className="relative z-10 mt-6 w-full max-w-[313px] self-center md:mt-[45px]">
...
const DOTTED_PANEL =
    'overflow-hidden rounded-[20px] bg-white/[0.05] p-2.5 [background-image:radial-gradient(rgba(255,255,255,0.07)_1px,transparent_1px)] [background-size:14px_14px]';

const MOBILE_SLIDES = ['Pre-qualify', 'Every path', 'Programs', 'Founder', 'Closing'];

// Phones get one program card at a time (swipe or tap the arrows) instead of a long stack of tall
// cards, so each one fits the screen and nothing is cut off.
function MobileCarousel() {
    ...
    const handleScroll = () => {
        ...
        if (track) {
            setActive(Math.round(track.scrollLeft / (track.children[0] as HTMLElement).offsetWidth));
        }
    };

    const slides = [
        <div key="prequal" className="flex w-full overflow-hidden rounded-[20px] [&>*]:w-full">
            <PrequalCard />
        </div>,
        <div key="every" className={cn('flex w-full [&>*]:flex-1', DOTTED_PANEL)}>
            <EveryPathCard />
        </div>,
        <div key="matched" className="flex w-full [&>*]:w-full">
            <MatchedCard />
        </div>,
        <div key="chat" className="flex w-full [&>*]:w-full">
            <ChatTestimonial />
        </div>,
        <div key="closing" className="flex w-full [&>*]:w-full">
            <ClosingCard />
        </div>,
    ];
    ...
    return (
        <div className="mt-[54px]">
            <div
                ...
                className="flex snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
            >
                {slides.map((slide, index) => (
                    <div key={MOBILE_SLIDES[index]} className="flex w-full shrink-0 snap-center">
                        {slide}
                    </div>
                ))}
            </div>
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
                <button
                    ...
                    className="grid size-11 place-items-center rounded-full bg-white/10 text-white ring-1 ring-white/15 transition-opacity disabled:opacity-30"
                >
                    <ChevronLeft className="size-5" />
                </button>

                <div className="flex flex-col items-center gap-2">
                    <span className="text-sm font-medium text-white">{MOBILE_SLIDES[active]}</span>
                    <div className="flex gap-1.5">
                        {MOBILE_SLIDES.map((label, index) => (
                            <button
                                key={label}
                                type="button"
                                onClick={() => goTo(index)}
                                aria-label={`Go to ${label}`}
                                className={cn('h-1.5 rounded-full transition-all duration-300', index === active ? 'w-6 bg-primary' : 'w-1.5 bg-white/25')}
                            />
                        ))}
                    </div>
                </div>
                ...
                <button
                    type="button"
                    onClick={() => goTo(Math.min(MOBILE_SLIDES.length - 1, active + 1))}
                    disabled={active === MOBILE_SLIDES.length - 1}
